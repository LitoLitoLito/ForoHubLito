package com.lito.foro_hub.controller;

import com.lito.foro_hub.model.usuario.*;
import com.lito.foro_hub.model.usuario.validaciones.actualizar.ValidarActualizarUsuario;
import com.lito.foro_hub.model.usuario.validaciones.crear.ValidarCrearUsuario;
...
public class UsuarioController {
    ...
    public ResponseEntity<String> eliminarUsuario(@PathVariable String username) {
        ...
        return ResponseEntity.ok("El usuario con username '" + username + "' ha sido deshabilitado exitosamente.");
    }
    @PutMapping("/activar/{username}")
    @Transactional
    @Operation(summary = "Habilita a un usuario deshabilitado")
    public ResponseEntity<String> activarUsuario(@PathVariable String username) {
        Usuario usuario = (Usuario) repository.findByUsername(username);

        if (usuario.getEnabled()) {
            return ResponseEntity.badRequest().body("El usuario con username '" + username + "' ya está habilitado.");
        }

        usuario.setEnabled(true);
        return ResponseEntity.ok("El usuario con username '" + username + "' ha sido habilitado exitosamente.");
    }
